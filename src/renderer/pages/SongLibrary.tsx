...
const SongLibrary = () => {
  ...
  return (
    <div>
      ...
      <div ref={parent} className="flex flex-col gap-y-5 my-6">
        {s.length === 0 && p.length === 0 && (
          <div>You haven't added any songs or playists..</div>
        )}
        {notFound && (
          <div>No songs and playlists with the name {search} were found</div>
        )}
        {(selection === 0 || selection === 1) && (
          <>
            {songs.map((el, i) => (
              <SongPlaylistCard {...el} expanded={true} />
            ))}
          </>
        )}
